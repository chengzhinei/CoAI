...
import (
	...
	evmtypes "github.com/myuser/CoAI/types"
	"github.com/myuser/CoAI/x/evm/keeper"
	"github.com/myuser/CoAI/x/evm/types"
)
...
func InitGenesis(
	...
	for _, account := range data.Accounts {
		...
		ethAcct, ok := acc.(evmtypes.EthAccountI)
		if !ok {
			panic(
...
func ExportGenesis(ctx sdk.Context, k *keeper.Keeper, ak types.AccountKeeper) *types.GenesisState {
	...
	ak.IterateAccounts(ctx, func(account sdk.AccountI) bool {
		ethAccount, ok := account.(evmtypes.EthAccountI)
		if !ok {
